...
public class BoardDAO {
	...
	public Object list(int page, int limit) {
		...
		try {
			...
			sql = "select * from board order by gid desc, seq  limit ?, ?"; // 리미트 이용해서 일부만 추출
	...
	public void write(BoardDTO dto) {
		...
		try {
			...
			sql = "insert into board " + "(seq, level, no, title, pname, pw, content, upfile, regdate ) values "
					+ "(0    ,  0  ,-1,  ? ,  ? ,   ? ,  ? , ? ,  sysdate() )";
	...
	public boolean modify(BoardDTO dto) {
		...
		try {
			...
			sql = "update board set title = ?, pname = ?, content = ?, upfile=?, no = no-1 where bid = ? and pw = ? ";
	...
	public void reply(BoardDTO dto) {
		...
		try {
			...
			sql = "insert into board " + "(gid, seq, level, no, title, pname, pw, content, regdate ) values "
					+ "( ? ,  ? ,  ?   , -1,   ?  ,   ?  , ? ,  ?    , sysdate())";
